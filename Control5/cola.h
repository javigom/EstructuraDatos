...
public:
	...
	void llevarAlPrincipio(unsigned int pos) {
		...
		//Si no es una posición válida, lanzo la excepción
		if (pos > this->_nelems || pos < 1) {
			throw EPosicionInvalida();
		}

		else {
			//Si es el primer elemento, no hago nada ya que está al principio
			if (pos != 1) {
				int contador = 1;
				Nodo* ant = this->_prim;
				...
				//Desengancho el nodo con la posición deseada
				Nodo* aMover = ant->_sig;
				ant->_sig = aMover->_sig;

				//Coloco el nodo en la primera posición
				aMover->_sig = _prim;
				_prim = aMover;

				//Si es el último elemento, tengo que actualizar el valor de _ult
				if (pos == this->_nelems) {
					_ult = ant;
				}
			}

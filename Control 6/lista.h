...
public:
	...
	void enredar() {
		...
		//Si la cadena tiene menos de dos caracteres, no tengo que hacer nada
		if (longitud() > 2) {
			aMover = _ult;
			anterior = _prim;
			siguiente = _prim->_sig;
		}
			
		while (longitud() - contador > 2) {

			_ult = aMover->_ant;
			
			//_ult->_sig = NULL
			aMover->_ant->_sig = NULL;
			
			//aMover->_ant = anterior
			aMover->_ant = siguiente->_ant;
			
			//aMover->_sig = siguiente
			aMover->_sig = anterior->_sig;
			
			anterior->_sig = aMover;
			siguiente->_ant = aMover;
